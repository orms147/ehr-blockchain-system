...
interface IEHRSystem {
    ...
    event AccessRequested(bytes32 indexed reqId, address indexed requester, address indexed patient, string rootCID, RequestType reqType, uint40 expiry);
    event RequestApprovedByRequester(bytes32 indexed reqId, address indexed requester, uint40 timestamp);
    event RequestApprovedByPatient(bytes32 indexed reqId, address indexed patient, uint40 timestamp);
    event RequestCompleted(bytes32 indexed reqId, address indexed requester, address indexed patient, RequestType reqType);
    ...
    function confirmAccessRequestWithSignature(bytes32 reqId, bytes calldata signature) external;
