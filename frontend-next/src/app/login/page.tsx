...
export default function LoginPage() {
    ...
    // Handle post-connection authentication - only after user action
    useEffect(() => {
        const authenticateWithBackend = async () => {
            // Only authenticate if user clicked login (step === 2) AND is connected AND has provider
            if (!isConnected || !provider || step !== 2 || loading === false) {
                return;
            }
            ...
            try {
                ...
                if (loginResult.token) {
                    ...
                    setTimeout(() => {
                        router.push(loginResult.roles?.isVerifiedDoctor ? '/dashboard/doctor' : '/dashboard/patient');
                    }, 1500);
                }
            } catch (error: any) {
    ...
    }, [isConnected, provider, step, loading, disconnect, router, userInfo?.name]);
