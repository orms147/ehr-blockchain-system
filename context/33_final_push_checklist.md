...
> - **k6 #4**: ✅ XONG (đo thật 2026-06-24, k6 v2.0.0). 3 kịch bản production-like (tắt query-log, pool 50): Nhẹ 10VU p95=300ms/4 RPS, Vừa 50VU p95=287ms/27 RPS, Căng 200VU p95=925ms/184 RPS — **0% lỗi cả 3**. Max-throughput: trần ~190 RPS. 429: 52% bị chặn @1ms. **Neon dashboard: DB KHÔNG bão hoà** (CPU ~25%, conn ~30-40/450, pooler 10000, 0 deadlock/wait) → trần = RTT VN-Singapore + đo local 1 tiến trình, KHÔNG phải DB/kiến trúc. Đã điền Bảng 4.7 + viết lại 4.4.2 (b/c/d/e/f) + **sửa "3-5 kết nối" (SAI) → số thật** ở Ch2/Ch3/Ch4/Ch6/Tóm tắt/README. Artifacts: `backend/scripts/mint-test-jwt.js`, `loadtest/max-throughput.k6.js`, `load-test/results/*-tuned.summary.json`. Temp config (rate-limit, pool, query-log) đã REVERT, git diff backend = 0.
> - **CÒN LẠI (owner-driven, không phải viết text):** (1) **Sơ đồ #10** — render `figures/*.puml`→`Hinhve/*.png` (cần Java+PlantUML; chưa cài) + sửa nội dung .puml theo tên hàm/state thật; (2) **F3** điền cá nhân (Bia/Bia_lot/Loi_cam_on/Phu_luc_A:100 demo accounts); (3) optional: ct17_ttg thiếu year (chưa web-verify — không bịa); (4) optional: đo thêm đường đọc-chéo có RPC canAccess.
