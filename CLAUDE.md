...
## 🔴 RULE #0 — QUY TẮC MẠNH NHẤT, ƯU TIÊN TUYỆT ĐỐI

> **CHECK CODE TRƯỚC KHI TRẢ LỜI. CHỈ NÓI ĐIỀU CÓ NGUỒN RÕ RÀNG. KHÔNG BỊA.**
>
> - Trước khi khẳng định bất cứ điều gì về code/hành vi hệ thống, **phải đọc file thật** và dẫn nguồn cụ thể (`path:line`, output lệnh đã chạy, hoặc doc trong `context/`).
> - Không suy đoán, không "chắc là", không dựa vào trí nhớ/CLAUDE.md cho chi tiết kỹ thuật — **code là nguồn chân lý** (CLAUDE.md có thể stale).
> - Nếu chưa verify hoặc không tìm thấy nguồn → **nói thẳng "chưa kiểm chứng / không rõ"**, không chế ra câu trả lời nghe có vẻ đúng.
> - Số liệu (line number, địa chỉ contract, tên field, hàm...) phải khớp file hiện tại; nếu nghi ngờ thì mở file kiểm tra lại.

> ⚠️ **PARTIALLY STALE (body refreshed 2026-06-22).** Khôi phục từ git history sau mất data 2026-06-18.
> Đã sửa các mục stale chính bên dưới (§3/§7/§12). Vẫn lưu ý các drift đã biết:
> - `includeUpdates` đã **bỏ** (medical-episode model) — §5 vẫn còn bảng cũ dùng cột này, đọc với lưu ý.
> - emergency-witness/`grantEmergencyAccess` đã **bỏ** → **Trusted Contact registry** (`ConsentLedger.setTrustedContactBysig`/`isTrustedContact`).
> - chỉ **1 worker `startSubgraphSync`** chạy (3 worker RPC `eventSync`/`recordRegistrySync`/`consentLedgerSync` tồn tại nhưng KHÔNG start).
...
```
c:\Work\DATN\ehr-blockchain-system\
...
├── Bao Cao/        Quyển LaTeX (gitignored) + figures/*.puml
├── context/        Tài liệu kiến trúc + audit (00_index.md → 31_*.md)
└── docs/           Bộ hướng dẫn hệ thống (git-tracked) — 19 file
                    (frontend/ Next.js đã XOÁ)
```
...
| [DoctorUpdate.sol](contracts/src/DoctorUpdate.sol) | 274 | Facade `addRecordByDoctor` (modifier `onlyDoctor`=isDoctor, KHÔNG verified; không check parent consent). `grantEmergencyAccess` ĐÃ BỎ → Trusted Contact ở ConsentLedger |
...
Interfaces: [contracts/src/interfaces/](contracts/src/interfaces/) — `IAccessControl`, `IConsentLedger` (chứa `struct Consent` với `allowDelegate`; `includeUpdates` ĐÃ BỎ 2026-04-19), `IRecordRegistry`, `IEHRSystemSecure`.
...
- `subgraphSync.service.js` — **worker DUY NHẤT đang chạy** (poll The Graph 30s → DB cache; started bởi `app.js` qua `startSubgraphSync()`). Các file `eventSync` / `recordRegistrySync` / `consentLedgerSync` còn tồn tại (subgraphSync tái dùng handler của chúng) nhưng KHÔNG được start riêng (RPC polling tắt từ 2026-04-30, S17). `DoctorVerified` finalize `verificationRequest` trong vòng lặp doctor của subgraphSync.
...
- `User` (wallet auth + NaCl pubkey + quota), `RecordMetadata`, `KeyShare` (có `allowDelegate`; KHÔNG có `includeUpdates` lẫn `source` — `source` nằm ở `KeyShareMutationLog`)
...
- `backend/src/services/ipfs.service.js` — mock CID (CHỈ backend). Mobile `mobile/src/services/ipfs.service.js` upload Pinata THẬT (`pinFileToIPFS`). NGOẠI LỆ: `CredentialSubmitScreen` (tài liệu CCHN) vẫn dùng mock CID `mock-…`, chưa upload ảnh thật.
- Ministry dashboard v2 (`screens-v2/ministry/*`) là THẬT (tạo org + verify doctor + pause/revoke), KHÔNG còn "60% mock".
- Push notification ĐÃ implement (mobile `notifications.ts` wire ở `App.tsx`; backend `push.service.js` dùng khắp nơi) — KHÔNG còn "0%".
